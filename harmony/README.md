...
- 本地 `142` 个逻辑/契约测试通过，API 26 unsigned/signed HAP 构建通过。
...
- 数据：RDB 幂等迁移、参数化 upsert、远端 ID 校验、跨设备 LWW、非破坏全拉与安全全量重传、服务器游标、墓碑删除、退避重试、WorkScheduler 后台恢复。
...
- 胶囊安全：恢复码须经 24 词/词表/v3 试解；BTC2/BTC3 具备 iOS CryptoKit、独立 Node AES-GCM 与 Harmony 启动 CryptoFramework 共用 golden vector。
...
- 启动保护：RDB 迁移先查 schema、整批事务并断言关键列；真实 ALTER 错误进入独立保护页，不加载正常写入、同步或新人引导。
...
- 体验：长辈模式、家庭成员管理、正式布布吉祥物典礼、深色、减少动态、提示音、WCAG AA 语义色、核心朗读语义、相框模式。
