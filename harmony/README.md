...
- 本地 `113` 个逻辑/契约测试通过，API 26 unsigned/signed HAP 构建通过。
...
- 数据：RDB 幂等迁移、参数化 upsert 回写、远端 ID 校验、全部集合双向同步、服务器游标、墓碑删除、退避重试、WorkScheduler 后台恢复。
- 媒体：文件流式上传、真实进度、跨端远端缩略图优先、原片后台补拉、资源角色分组、人脸数量、认布布提示、端侧物体标签、照片收件箱、移动硬盘候选确认、详情多媒体左右翻页、鉴权补拉、照片缩放平移、存相册与 Share Kit 分享。
...
- 提醒：设置页可开关 30 天“那年今日”滚动提醒，并自动安排最近三针疫苗的预告和到期提醒。
...
- 体验：可手动开启的姥姥三动作模式、家庭成员切换/编辑/安全删除、系统/星夜深色、核心屏幕朗读语义、旧手机全屏相框模式。
