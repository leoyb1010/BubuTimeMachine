...
- Storybook stories and tests: 不适用 ArkUI；当前 105 个 Node 契约/逻辑测试通过，新增成长电影 jobId 持久化、续轮询、十分钟窗口和连续失败容错门禁
...
**当前状态：HOLD** — 当前版本通过 105 项测试与 API 26 signed 构建；成长电影任务离页或杀进程后不会丢失，回来继续取成片；设备当前 `[Empty]`，35MB HAP SHA-256 `2575c8a51f32bf256e1b3553ce5cbdb8c753d9118dbd43f7e4c89113cea6de9b`，必须重连安装后执行媒体页码、缩略图与逐层返回断言才能转 PASS。
