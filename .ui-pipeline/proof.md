...
- Storybook stories and tests: 不适用 ArkUI；当前 128 个 Node 契约/逻辑测试通过，新增安全全量重传、清库 API 移除、相框批量媒体查询和资源角色过滤门禁
...
**当前状态：HOLD** — 当前版本通过 128 项测试与 API 26 signed 构建；同步中心支持非破坏全拉和 LWW 保护的全量重传，生产代码不再保留清库入口；设备当前 `[Empty]`，35MB HAP SHA-256 `9d58edbeae395f3bf4760c6b7c2887b0332353803da14b3d9f4e7d3bdecfd89b`，必须重连安装后执行媒体页码、缩略图、大视频和逐层返回断言才能转 PASS。
