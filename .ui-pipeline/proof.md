...
- Storybook stories and tests: 不适用 ArkUI；当前 96 个 Node 契约/逻辑测试通过，新增数据库初始化失败独立保护页与禁止正常 Root 挂载门禁
...
**当前状态：HOLD** — 当前版本通过 96 项测试与 API 26 signed 构建；数据库失败时不会进入新人引导或任何正常写路径；设备当前 `[Empty]`，35MB HAP SHA-256 `686951f1f066f97dff36c60388548bafb83eebed1085538e8d378af0ed7ad17b`，必须重连安装后执行媒体页码、缩略图与逐层返回断言才能转 PASS。
